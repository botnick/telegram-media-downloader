...
function _items(file) {
    ...
    const out = [
        { id: 'open', icon: 'ri-eye-line', label: i18nT('gallery.context.open', 'Open') },
        {
            ...
            label: i18nT('gallery.context.download', 'Download original'),
        },
        { id: 'copy', icon: 'ri-link', label: i18nT('gallery.context.copy', 'Copy link') },
        {
            id: 'share',
            icon: 'ri-share-line',
            label: i18nT('gallery.context.share', 'Share link'),
            adminOnly: false,
        },
    ];
    if (isAdmin) {
        ...
        out.push({
            ...
            label: file?.pinned
                ? i18nT('gallery.context.unpin', 'Unpin')
                : i18nT('gallery.context.pin', 'Pin'),
        });
        out.push({
            id: 'forward',
            icon: 'ri-share-forward-line',
            label: i18nT('gallery.context.forward', 'Forward'),
        });
        out.push({ id: 'sep2', divider: true });
        out.push({
            id: 'delete',
            icon: 'ri-delete-bin-line',
            label: i18nT('gallery.context.delete', 'Delete'),
            danger: true,
        });
    }
    return out;
...
async function _handle(action, file, idx) {
    ...
    if (action === 'copy') {
        ...
        try {
            ...
            showToast(
                i18nT('gallery.context.copy_failed', 'Could not copy — paste from the address bar'),
                'error',
            );
        }
        return;
    ...
    if (action === 'share') {
        ...
        if (file.id == null) {
            showToast(
                i18nT('gallery.context.share_no_id', 'This row has no DB id — share not available'),
                'error',
            );
            return;
        }
    ...
    if (action === 'pin') {
        ...
        try {
            ...
            const tile = document.querySelector(
                `.media-item[data-id="${CSS.escape(String(file.id))}"]`,
            );
    ...
    if (action === 'forward') {
        ...
        if (file.id == null) {
            showToast(
                i18nT('gallery.context.forward_unavailable', 'Forward not available'),
                'error',
            );
            return;
        }
    ...
    if (action === 'delete') {
        ...
        const ok = await confirmSheet({
            ...
            message: i18nT(
                'gallery.context.delete_confirm',
