...
describe('_parseChecksumFile', () => {
    ...
    it('parses sha256sum format with filename', async () => {
        ...
        await expect(_parseChecksumFile(`${hex}  tgdl-seekbar-linux-x64.tar.gz`)).resolves.toBe(
            hex,
        );
    });

    it('normalises uppercase hex to lowercase', async () => {
        await expect(_parseChecksumFile('A'.repeat(64))).resolves.toBe('a'.repeat(64));
    });
...
describe('_hashFile', () => {
    it('returns SHA-256 hex of a known file', async () => {
        const content = Buffer.from('hello seekbar\n');
        const tmp = path.join(os.tmpdir(), `tgdl-seekbar-test-hash-${Date.now()}.bin`);
...
describe('_verifyChecksum', () => {
    it('resolves when checksum matches', async () => {
        const content = Buffer.from('seekbar binary payload');
        const tmp = path.join(os.tmpdir(), `tgdl-seekbar-verify-${Date.now()}.bin`);
        await fs.writeFile(tmp, content);
        serveStatus = 200;
