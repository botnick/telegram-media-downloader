...
[![Tests](https://img.shields.io/badge/tests-1219%20passing-brightgreen)](https://github.com/botnick/telegram-media-downloader/actions)
...
> [Quick start](#quick-start) · [Whatʼs new in v2.10](#whats-new-in-v2100) · [Cluster mode](docs/CLUSTER.md) · [Architecture](docs/ARCHITECTURE.md) · [API reference](docs/API.md) · [Deploy](docs/DEPLOY.md) · [Backup providers](docs/BACKUP.md) · [Troubleshooting](docs/TROUBLESHOOTING.md) · [AI subsystem](docs/AI.md) · [Migration v2.9 → v2.10](docs/MIGRATION-v2.9-to-v2.10.md) · [Audit](docs/AUDIT.md)

## What's new in v2.10.0

- **🌐 Cluster mode** — federate two or more dashboards into one library. Per-peer tokens, real-time WebSocket sync (sub-second), LAN auto-discovery, owner-peer routing, **automatic backup-peer failover**, relay-through-peer for any-NAT setups, live config replication, cross-peer dedup, and a unified gallery + search across every paired instance. See [`docs/CLUSTER.md`](docs/CLUSTER.md).
- **🔄 Auto-update overhaul** — pre-flight watchtower ping, live SQLite `quick_check`, snapshot **with verify-by-reopen**, structured `error_code` per failure, full `update_history` audit page, configurable stall timeout, automatic backup pruning. Survives `:latest`-tag rebuilds via boot-instance-id rotation.
- **🤖 AI Doctor card** — one-shot health probe (`/api/ai/health`) for sharp / Transformers.js / sqlite-vec / models cache. Missing libvips, wrong libc, or stale Node ABI no longer crash the AI subsystem at boot — capabilities fail cleanly with platform-aware remediation text.
- **🔐 Re-auth modal** — session expiry now pops a modal in the SPA instead of bouncing you to `/login.html`. Single-flight queueing, graceful redirect fallback. Stay where you are, sign back in, resume.
- **🛠️ Maintenance hub polish** — Find duplicates page gets a top stats panel (Total / Hashed / Awaiting / Last scan), a Verify-files button (was buried in Settings), bigger progress bars with stage-specific labels, and a persisted "Last scan / Last build / Last run" line that survives server restart. Same UX bar applied to thumbnails, video faststart, and NSFW pages.
- **🏗️ Infrastructure** — `autoheal` Docker sidecar restarts wedged event loops; per-container memory ceilings + log rotation in the bundled compose file; PM2 ecosystem retargeted at the dashboard with a 1.5 GB OOM guard.
- **🐛 Race-condition fix** — every fire-and-forget admin route (`thumbs/build-all`, `faststart/scan`, `dedup/scan`, `reindex`) migrated to the shared `JobTracker`. The double-click 409 after a failed run is gone.
- **🌐 i18n drift checker** — `npm run i18n:check` (or `node scripts/check_i18n_drift.js`) catches missing translation keys before a release ships.
- **🧪 1219 specs passing** across 154 test files. New cluster (14 files — identity, hmac, handshake, peers, sync, dedup, sweep, config-sync, discovery, failover, proxy, relay, tokens, e2e), AI (4 — health, lazy-sharp, routes, safe-route), update (3 — updater, update-history, update-routes), reauth-modal (1), and maintenance race + persistence (2) suites.

Full ship list in [CHANGELOG.md](CHANGELOG.md). Cluster operators upgrading from v2.9: see [docs/MIGRATION-v2.9-to-v2.10.md](docs/MIGRATION-v2.9-to-v2.10.md).
...
- **Local AI search & smart organisation** — opt-in CLIP semantic search ("show me beach photos"), face clustering (the **People** view), perceptual near-duplicate dedup, and ImageNet auto-tagging. Everything runs locally via WASM; no cloud APIs, no uploads. New AI Doctor card surfaces missing native deps with platform-aware fix-it text. See [docs/AI.md](docs/AI.md).
